...
- Apache config: write managed block markers (`# BEGIN qnap-docker-mdns managed`
  ... `# END qnap-docker-mdns managed`) into the appropriate per-port file
  (`/etc/reverseproxy/extra/80.conf`) for reliable routing.
- JSON config: write matching entries into `reverseproxy.json` with
  `(managed)` in the `name` field so rules appear in the QNAP UI.
- Graceful reload via `reverseproxy -k graceful -f /etc/reverseproxy/reverseproxy.conf`.
- On reconciliation: re-verify both JSON entries and the managed block exist
  and are consistent; rewrite whichever is missing.
...
- If a requested primary hostname collides with an unmanaged reverse proxy `ServerName` or `ServerAlias` already present outside the managed block, the container must be skipped as actionable misconfiguration.
- If a requested alias collides with an unmanaged reverse proxy `ServerName` or `ServerAlias` already present outside the managed block, skip only the colliding alias and continue publishing the remaining non-conflicting names for that container.

---

## Generated Configuration

The daemon writes to two locations per reconciliation:

### Apache per-port file (routing)

The managed block is written directly into the per-port file (e.g.,
`/etc/reverseproxy/extra/80.conf`), delimited by markers:

```apache
# BEGIN qnap-docker-mdns managed
# qnap-docker-mdns: grafana.local (managed)
...
# END qnap-docker-mdns managed
```

Only the managed section should be modified.
...
The daemon keeps two representations of its managed rules in sync on every
reconciliation pass.
...
If markers do not exist (e.g. after `scan_config` wiped the file):

1. Read the current file and remove any QNAP-generated VirtualHost entries
   whose `ServerName` matches one of the daemon's managed rules. These entries
   were generated from the daemon's JSON entries during `scan_config` and must
   be removed to prevent duplicate VirtualHost blocks.
2. Append managed section with markers.
...
- All files in `/etc/reverseproxy/extra/` are deleted and regenerated from JSON.
- The managed block markers are lost.
- The JSON entries survive (if QNAP's CGI preserved them) and become part of
  the generated per-port file.
- On the next reconciliation pass, the daemon detects the missing markers,
  removes the QNAP-generated duplicates from the per-port file (by matching
  `ServerName`), re-writes the managed block, re-verifies the JSON entries,
  and reloads.

### Uninstall

1. Remove daemon entries from `reverseproxy.json`.
2. Empty the managed block in the per-port file, leaving unmanaged entries unchanged.
...
Before first modification to a per-port file:

```bash
cp /etc/reverseproxy/extra/80.conf \
   /etc/reverseproxy/extra/80.conf.qnap-docker-mdns.bak
```

Never overwrite backup automatically.

The backup exists for disaster recovery and troubleshooting, not as the primary uninstall path.
...
1. Watches Docker.
2. Builds registry.
3. Generates VirtualHosts.
4. Writes managed block.
5. Reloads proxy.
...
reverse_proxy:
  ...
  reload_command: /usr/local/apache/bin/reverseproxy -k graceful -f /etc/reverseproxy/reverseproxy.conf
  validate_command: /usr/local/apache/bin/reverseproxy -t -f /etc/reverseproxy/reverseproxy.conf
...
```bash
notice_log_tool \
  --append "[qnap-docker-mdns] reverse proxy reload failed: exit_status=1 command=/etc/init.d/reverse_proxy.sh restart" \
  --severity 3 \
  --user admin \
  --serviceName qnap-docker-mdns \
  --facility 13
...
```bash
logger -t qnap-docker-mdns -p daemon.err "reverse proxy reload failed: exit_status=1 command=/etc/init.d/reverse_proxy.sh restart"
```
...
- managed block renderer for the Apache per-port file
- JSON rule renderer for the QNAP config database
- template or rendering function for vhost entries
...
- ensure rendered `ProxyPass` and `ProxyPassReverse` targets always use `http://localhost:<port>/`
...
- sort rendered output deterministically so the file does not churn unnecessarily
- compare regenerated output against the current managed block to detect no-op reconciliations
...
- uninstall reconciliation path that removes JSON entries and empties the
  managed block while preserving user-managed config

Exit criteria:

- unmanaged configuration remains intact
- managed block in the per-port file and JSON database are always consistent
...
- before writing the managed block, read the per-port file and remove any
  ...
  duplicates generated by QNAP's `scan_config` from our JSON entries)
- write updated config through a temp file and atomic rename path
